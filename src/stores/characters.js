...
export const useCharactersStore = defineStore('characters', () => {
  ...
  const races = {
    human: {
      ...
      description: 'С шансом 2-из-6 люди автоматически замечают скрытые ловушки в <b>ярко освещённой</b> области радиусом 40\'. Кроме того, они получают на 10% больше ОО.'
    },
    gmur: {
      name: 'Гмур',
    ...
    div: {
      ...
      description: 'С шансом 2-из-6 дивьи автоматически замечают скрытые магией проходы и секретные двери в <b>ярко освещённой</b> области радиусом 40\'. Если при создании персонажа у него есть очки РЗМ, игрок может выбирать книги заклинаний, вместо того чтобы определять их случайным образом.'
    }
  }
  ...
  // Удалить персонажа или транспорт
  ...
  async function createVehicle(campaignId, vehicleData, userId) {
    loading.value = true
    error.value = null
    ...
    try {
      ...
      return { id: docRef.id, ...vehicleData, type: 'vehicle' }
    } catch (e) {
      error.value = e.message
      throw e
    } finally {
  ...
  function getMyCharacters(userId) {
    return characters.value.filter(c => c.ownerId === userId && c.type !== 'vehicle')
  }
